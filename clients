...
def largest_earnings_per_day(filename: str) -> Optional[Client]:
    ...
    """
    clients = read_from_file_into_list(filename)
    clients_with_earnings = [client for client in clients if client.current_amount > client.starting_amount]
    if not clients_with_earnings:
        return None

    return max(clients_with_earnings, key=lambda client: (client.earnings_per_day(), -client.account_age))
    pass
...
def largest_loss_per_day(filename: str) -> Optional[Client]:
    ...
    """
    clients = read_from_file_into_list(filename)
    clients_with_losses = [client for client in clients if client.current_amount < client.starting_amount]

    if not clients_with_losses:
        return None

    return min(clients_with_losses, key=lambda client: (client.earnings_per_day(), client.account_age))
    pass
